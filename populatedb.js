console.log(
    'This script populates some test books, authors, genres, and bookinstances to your database. Specified database as an argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.lz91hw2.mongodb.net/local_library?retryWrites=true&w=majority"'
  );
  ...
  async function authorCreate(index, firstName, lastName, birthDate, deathDate) {
    try {
      const authorDetail = { firstName, lastName };
      if (birthDate) authorDetail.birthDate = birthDate;
      if (deathDate) authorDetail.deathDate = deathDate;
  ...
  async function bookInstanceCreate(index, book, imprint, dueBack, status) {
    try {
      ...
      if (dueBack) bookInstanceDetail.dueBack = dueBack;
      ...
      console.error('Error creating book instance:', error.message);
      throw error;
    }
  }
...
// const { MongoClient, ServerApiVersion } = require('mongodb');
// const uri = "mongodb+srv://admin_1:80vJPUZdbqvqTMrS@cluster0.ubcgjdy.mongodb.net/?retryWrites=true&w=majority";
